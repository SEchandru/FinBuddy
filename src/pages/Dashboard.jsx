...
function Dashboard() {
  ...
  const assetNetWorth = currentSavings + currentInvestments;
  const recommendedAllocations = [
    { name: 'Emergency Fund', value: cashPct, rupee: Math.round((cashPct/100)*assetNetWorth) },
    { name: 'Equity Mutual Funds', value: equityPct, rupee: Math.round((equityPct/100)*assetNetWorth) },
    { name: 'Debt Funds/Bonds', value: debtPct, rupee: Math.round((debtPct/100)*assetNetWorth) },
    { name: 'Gold / Hedging', value: goldPct, rupee: Math.round((goldPct/100)*assetNetWorth) }
  ];
  ...
  return (
    <div className="p-8 bg-slate-950 min-h-screen text-slate-100 space-y-8 font-sans">
      ...
      {/* SECTION 1: HERO FINANCIAL HEALTH PANEL */}
      <div className="bg-gradient-to-br from-slate-900/60 via-slate-900/40 to-slate-950 border border-slate-800/80 rounded-2xl p-8 shadow-2xl relative overflow-hidden backdrop-blur-xl">
        <div className="absolute top-0 right-0 h-48 w-48 rounded-full bg-blue-500/5 blur-3xl"></div>
        
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="space-y-3 lg:max-w-xl">
            <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest px-2.5 py-1 rounded-full bg-blue-950/40 border border-blue-900/30">
              {stageName} {stageDesc}
            </span>
            <h1 className="text-4xl font-extrabold tracking-tight text-white pt-1">
              Welcome back, {user?.name || 'User'} 👋
            </h1>
            <p className="text-slate-400 text-xs leading-relaxed">
              Based on your profile, here is your financial health status. You are currently in the stage of building capital and protecting dependencies.
            </p>
            
            {/* Strengths & Weaknesses Brief */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 text-xs">
              <div className="space-y-1">
                <span className="text-slate-500 font-bold block uppercase text-[9px] tracking-wide">Key Strengths</span>
              ...
              <div className="space-y-1">
                <span className="text-slate-500 font-bold block uppercase text-[9px] tracking-wide">Primary Vulnerability</span>
                <ul className="space-y-1 text-slate-300">
                  {weaknesses.slice(0, 1).map((w, idx) => (
                    <li key={idx} className="flex items-start gap-1.5 text-rose-400 font-semibold">
                      <FaTimesCircle className="shrink-0 text-[10px] mt-0.5" /> <span className="text-slate-300 font-medium">{w}</span>
                    </li>
                  ))}
                  {weaknesses.length === 0 && <span className="text-emerald-400 font-medium">None identified!</span>}
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center bg-slate-950/40 border border-slate-850 p-6 rounded-2xl shrink-0 text-center lg:w-56">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-3">Finance Health Score</span>
            <div className="h-28 w-28 rounded-full border-4 border-slate-850 bg-slate-950 flex flex-col items-center justify-center relative overflow-hidden mb-2">
            ...
            <div className={`text-base font-extrabold flex items-center justify-center gap-1 ${ratingColor}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-4">
            <div>
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <FaCompass className="text-yellow-400" /> What Should I Do Next?
              </h2>
              <p className="text-xs text-slate-400 mt-1">Clear guidance on your top 3 critical advisor priorities.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {top3Priorities.map((pri, idx) => (
                <div key={idx} className="bg-slate-950/80 border border-slate-850 p-5 rounded-xl flex flex-col justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-bold text-blue-400 uppercase tracking-widest bg-blue-950/40 border border-blue-900/20 px-2 py-0.5 rounded-full">
                        Priority {idx + 1}
                      </span>
                      <span className="text-[10px] text-slate-500 font-bold uppercase">{pri.timeline}</span>
                    </div>
                    <h3 className="text-sm font-bold text-slate-200">{pri.title}</h3>
                    <p className="text-[11px] text-slate-400 leading-normal">{pri.reason}</p>
                  </div>

                  <div className="pt-2 border-t border-slate-850 flex justify-between items-center text-xs">
                    <div>
                      <span className="text-[9px] text-slate-500 uppercase block font-bold">Target</span>
                      <strong className="text-slate-350">{pri.target}</strong>
                    </div>
                    <button
                      onClick={() => navigate(pri.path)}
                      className="h-7 w-7 rounded-lg bg-blue-600 hover:bg-blue-500 flex items-center justify-center text-white transition-all shadow-md shrink-0"
                    >
          ...
          {/* Grid: Roadmap & Checklist */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            
            {/* SECTION 3: FINANCIAL ROADMAP PROGRESS */}
            <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-4">
              <h3 className="text-sm font-bold text-teal-400 flex items-center gap-2">
                <FaMapSigns /> Financial Roadmap Progress
              </h3>
              
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-semibold">
                  <span className="text-slate-400">Roadmap Completion</span>
                  <span className="text-teal-400 font-bold">{roadmapProgressPct}% Completed</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs pt-2">
                {roadmapItems.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2 p-2 bg-slate-950/40 border border-slate-850/50 rounded-lg">
                    {item.completed ? (
                      <FaCheckCircle className="text-emerald-400 shrink-0 text-[10px]" />
                    ) : (
                      <FaTimesCircle className="text-slate-650 shrink-0 text-[10px]" />
                    )}
                    <span className={`font-semibold ${item.completed ? 'text-slate-350' : 'text-slate-500'}`}>{item.name}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* SECTION 4: LIFE STAGE CHECKLIST */}
            <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-3.5">
              <h3 className="text-sm font-bold text-violet-400 flex items-center gap-2">
                <FaClipboardList /> Life Stage: {stageName.replace(" Stage", "")}
              </h3>
              <p className="text-[10px] text-slate-400 leading-normal">Recommended safety checkpoints for age group {stageDesc}:</p>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-5">
            <h3 className="text-sm font-bold text-emerald-400 flex items-center gap-2">
              <FaChartPie /> Suggested Asset Allocation
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              ...
              <div className="space-y-2.5">
                {recommendedAllocations.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center text-xs bg-slate-950 p-3 border border-slate-850 rounded-xl">
                ...
                <p className="text-[10px] text-slate-450 leading-relaxed font-semibold italic pt-2">
              ...
              <div className="h-48 text-center flex flex-col items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {recommendedAllocations.map((entry, index) => (
                ...
                <div className="flex justify-center gap-3 text-[9px] text-slate-500 mt-2">
                  {recommendedAllocations.map((item, idx) => (
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-4">
            <div className="flex justify-between items-center border-b border-slate-850 pb-2">
              <h3 className="text-sm font-bold text-blue-400 flex items-center gap-2">
                <FaShieldAlt /> Protection Status
              </h3>
              <div className="text-right">
                <span className="text-[9px] text-slate-500 font-bold block uppercase">Protection Score</span>
                <strong className="text-sm text-blue-400">{protectionScoreNum} / 100</strong>
              </div>
            </div>

            <div className="space-y-2">
              {protectionItems.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-xs bg-slate-950 p-2.5 border border-slate-850 rounded-xl">
            ...
            {criticalMissingItems.length > 0 && (
              <div className="bg-red-500/5 border border-red-500/10 p-3.5 rounded-xl mt-3 text-[11px] text-red-400 flex items-start gap-2">
                <FaExclamationTriangle className="shrink-0 mt-0.5 text-xs animate-pulse" />
                <div>
                  <span className="font-bold uppercase text-[9px] block">Critical Missing Items</span>
                  {criticalMissingItems.join(', ')}
                </div>
              </div>
            )}
          </div>

          {/* SECTION 7: FIINBUDDY ADVISOR CARD */}
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-4">
            <h3 className="text-sm font-bold text-yellow-400 flex items-center gap-2">
              <FaLightbulb /> FiinBuddy Wealth Coach
            </h3>
            <p className="text-xs text-slate-300 leading-relaxed font-semibold italic">
              "{advisorMessage}"
            </p>

            {/* Warnings Suggestions Opportunities */}
            <div className="space-y-3.5 pt-2 text-xs leading-normal">
              {warningsList.length > 0 && (
                <div>
                  <span className="text-[9px] font-bold text-rose-500 uppercase tracking-wider block">⚠️ Advisor Warnings</span>
                  <ul className="list-disc pl-4 space-y-1 text-slate-350 mt-1 font-medium">
                    {warningsList.map((w, idx) => <li key={idx}>{w}</li>)}
                  </ul>
                </div>
              )}
              ...
              {suggestionsList.length > 0 && (
                <div>
                  <span className="text-[9px] font-bold text-blue-400 uppercase tracking-wider block">💡 Advisor Suggestions</span>
                  <ul className="list-disc pl-4 space-y-1 text-slate-350 mt-1 font-medium">
                    {suggestionsList.map((s, idx) => <li key={idx}>{s}</li>)}
                  </ul>
                </div>
              )}

              <div>
                <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-wider block">🚀 Wealth Opportunities</span>
                <ul className="list-disc pl-4 space-y-1 text-slate-350 mt-1 font-medium">
                  {opportunitiesList.map((o, idx) => <li key={idx}>{o}</li>)}
                </ul>
              </div>
            </div>
          </div>
      ...
      {/* SECTION 8: GOAL TRACKER & SECTION 9: EXPENSE SUMMARY */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* Goal Tracker */}
        <div className="lg:col-span-1 bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md flex flex-col justify-between gap-4">
          <div>
            <h3 className="text-sm font-bold text-blue-400 flex items-center gap-2 border-b border-slate-850 pb-2.5">
            ...
            <div className="space-y-3.5 mt-4 max-h-[220px] overflow-y-auto pr-1">
              {goals.slice(0, 3).map((goal) => {
                ...
                return (
                  <div key={goal.id} className="bg-slate-950/60 border border-slate-850 rounded-xl p-3.5 space-y-2">
                    <div className="flex justify-between text-xs font-semibold">
                      <span>{goal.name}</span>
                      <span className="text-teal-400 font-bold">{pct}%</span>
                    </div>
                    <div className="w-full bg-slate-850 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-gradient-to-r from-teal-500 to-blue-500 h-full rounded-full" style={{ width: `${pct}%` }}></div>
                    </div>
              ...
              {goals.length === 0 && (
                <div className="text-slate-500 text-xs py-8 text-center">
                  No active goals set. Set targets inside Goal Planner.
                </div>
              )}
            </div>
          </div>
          {goals.length > 0 && (
            <Link to="/goals" className="text-xs font-bold text-blue-400 hover:text-blue-300 flex items-center gap-1 mt-2">
              View all active goals <FaArrowRight className="text-[10px]" />
            </Link>
          )}
        </div>

        {/* Expense Summary & Budgets */}
        <div className="lg:col-span-2 bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-md flex flex-col justify-between gap-4">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <div className="bg-slate-950 p-4 border border-slate-850 rounded-xl">
                ...
                <span className="text-lg font-bold text-slate-200">₹{monthlySalary.toLocaleString()}</span>
              </div>
              <div className="bg-slate-950 p-4 border border-slate-850 rounded-xl">
                <span className="text-slate-500 block text-[9px] uppercase font-bold">This Month's Spending</span>
                <span className="text-lg font-bold text-red-400">₹{totalMonthlyExpenses.toLocaleString()}</span>
              </div>
              <div className="bg-slate-950 p-4 border border-slate-850 rounded-xl">
                <span className="text-slate-500 block text-[9px] uppercase font-bold">Actual Savings Rate</span>
                <span className={`text-lg font-bold ${parsedSavingsRate >= 20 ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {parsedSavingsRate}%
                </span>
              </div>
            </div>

            {/* Category spending list & trends */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-xs">
              <div className="bg-slate-950/60 p-4 border border-slate-850 rounded-xl space-y-2">
                <span className="text-slate-500 font-bold block uppercase text-[9px] tracking-wide">Top Spending Categories</span>
                {categoryBreakdown.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center py-1 border-b border-slate-900/50 last:border-0 text-slate-300">
                    <span className="font-semibold">{item.name}</span>
                    <strong className="text-rose-400">₹{item.value.toLocaleString()}</strong>
                  </div>
                ))}
                {categoryBreakdown.length === 0 && <span className="text-slate-500 block py-2">No transactions recorded this month.</span>}
              </div>

              <div className="bg-slate-950/60 p-4 border border-slate-850 rounded-xl space-y-1.5 flex flex-col justify-center">
                <span className="text-slate-500 font-bold block uppercase text-[9px] tracking-wide mb-1">Budget Health Status</span>
                <strong className={`text-lg font-black ${parsedSavingsRate >= 30 ? 'text-emerald-400' : parsedSavingsRate >= 20 ? 'text-blue-400' : 'text-rose-400'}`}>
          ...
          <Link to="/expenses" className="text-xs font-bold text-blue-400 hover:text-blue-300 flex items-center gap-1 mt-2">
            View transaction logs <FaArrowRight className="text-[10px]" />
          </Link>
        </div>
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
          <Link 
            to="/expenses" 
            className="p-4 bg-slate-950 border border-slate-850 hover:border-slate-700 rounded-xl text-center flex flex-col items-center justify-center gap-2 transition-all group"
          >
          ...
          <Link 
            ...
            className="p-4 bg-slate-950 border border-slate-850 hover:border-slate-700 rounded-xl text-center flex flex-col items-center justify-center gap-2 transition-all group"
          >
            <FaCoins className="text-pink-400 group-hover:scale-110 transition-all" />
            <span className="text-xs font-semibold text-slate-300">Update Assets</span>
          </Link>
          <Link 
            to="/reports" 
            className="p-4 bg-slate-950 border border-slate-850 hover:border-slate-700 rounded-xl text-center flex flex-col items-center justify-center gap-2 transition-all group"
